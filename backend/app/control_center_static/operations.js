(() => {
  ...
  const fmtInt = (value) => value === null || value === undefined
    ? "—"
    : new Intl.NumberFormat("da-DK", { maximumFractionDigits: 0 }).format(num(value));
  const fmtDkk = (value, digits = 2) => value === null || value === undefined
    ? "—"
    : new Intl.NumberFormat("da-DK", { minimumFractionDigits: digits, maximumFractionDigits: digits }).format(num(value));
  ...
  const fmtDateTime = (timestamp) => timestamp
    ? new Intl.DateTimeFormat("da-DK", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(Number(timestamp) * 1000))
    : "—";
  const tone = (status) => {
  ...
  function renderE2E(snapshot) {
    ...
    const target = $("#opsE2E");
    if (!target) return;
    const healthy = e2e.operational_status === "healthy";
    target.innerHTML = `
      <div class="ops-title-row"><div><span class="panel-eyebrow">END-TO-END</span><h4>${healthy ? "Kurvs driftskæde er sund" : "Driftskæden kræver opmærksomhed"}</h4></div>${badge(e2e.operational_status, e2e.operational_status)}</div>
  ...
  function renderFreshness(snapshot) {
    ...
    target.innerHTML = (snapshot.operations?.freshness || []).map((row) => `
      <div class="ops-row"><span class="ops-state-dot ${tone(row.health)}"></span><div><strong>${esc(row.name)}</strong><small>${esc(row.detail || (row.at ? `senest ${fmtDateTime(row.at)}` : "ingen timestamp"))}</small></div><b>${esc(fmtAge(row.age_seconds))}</b></div>
    `).join("");
  }
  ...
  function renderAlertLifecycle(snapshot) {
    const target = $("#opsAlerts");
    if (!target) return;
    const alerts = snapshot.alerts || [];
    if (!alerts.length) {
      target.innerHTML = `<div class="ops-row"><span class="ops-state-dot healthy"></span><div><strong>Ingen aktive alarmer</strong><small>Ingen igangværende alarm-episoder</small></div><b>healthy</b></div>`;
      return;
    }
    target.innerHTML = alerts.map((row) => `
      <div class="ops-row"><span class="ops-state-dot ${tone(row.severity)}"></span><div><strong>${esc(row.title)}</strong><small>først set ${esc(fmtDateTime(row.first_seen))} · ${esc(row.detail || "")}</small></div><b>${esc(fmtAge(row.duration_seconds))} · ${fmtInt(row.occurrences)}×</b></div>
    `).join("");
  }
  ...
  function renderTrends(snapshot) {
    ...
    const definitions = [
      ["core_ms", "Core latency", "ms"],
      ["mobile_ms", "Mobile latency", "ms"],
      ["luna_cost_dkk", "Luna spend", "kr."],
      ["coverage_degraded", "Degraded", "aviser"],
    ];
    target.innerHTML = definitions.map(([key, label, unit]) => {
      const rows = series[key] || [];
      const current = rows.at(-1);
      const first = rows[0];
      const delta = current && first ? Number(current.value) - Number(first.value) : null;
      return `<article class="surface ops-trend-card"><div><span>${esc(label)}</span><strong>${current ? `${Number(current.value).toLocaleString("da-DK", { maximumFractionDigits: 2 })} ${unit}` : "—"}</strong><small>${delta == null ? "ingen trend endnu" : `${delta >= 0 ? "+" : ""}${delta.toLocaleString("da-DK", { maximumFractionDigits: 2 })} over historikken`}</small></div>${sparkline(rows)}</article>`;
    }).join("");
  }
  ...
  function renderDegraded(snapshot) {
    ...
    if (!target) return;
    const data = snapshot.operations?.degraded_impact || {};
    target.innerHTML = `
      <div class="ops-impact-grid">
        <div><strong>${fmtInt(data.degraded_publications)}</strong><span>degraded</span></div>
        <div><strong>${fmtInt(data.customer_sensitive_publications)}</strong><span>potentielt pris/member-sensitive</span></div>
        <div><strong>${fmtInt(data.other_quality_publications)}</strong><span>øvrig kvalitet</span></div>
      </div>
      <p class="ops-copy">${esc(data.note || "")}</p>
      <div class="ops-reasons">${(data.top_reasons || []).length ? data.top_reasons.map((row) => `<div><span>${esc(row.reason)}</span><strong>${fmtInt(row.count)}</strong></div>`).join("") : '<span class="empty-state compact">Ingen aktuelle quarantine-årsager.</span>'}</div>
    `;
  }
  ...
  function renderOpenAIEvents(snapshot) {
    ...
    target.innerHTML = rows.length
      ? rows.map((row) => `<div class="ops-row"><span class="ops-state-dot cost"></span><div><strong>${esc(row.title)}</strong><small>${esc(row.detail || "")}</small></div><b>${row.cost_dkk != null ? `+${fmtDkk(row.cost_dkk, 4)} kr.` : "—"}</b></div>`).join("")
      : '<div class="empty-state compact">Ingen nye OpenAI-kald registreret siden Operations v2 blev aktiveret.</div>';
  }
  ...
  function renderData(snapshot) {
    ...
    target.innerHTML = `
      ...
      <article class="data-stat-card surface"><span>Identity-læring</span><strong>${fmtInt(identity.stored_rules)}</strong><small>persistent product identity state</small></article>
      <article class="data-stat-card surface"><span>Control Center historik</span><strong>${fmtBytes(storage.control_center_telemetry_bytes)}</strong><small>events, trends og alert-lifecycle</small></article>
    `;
  }
