...
def _validated(value: str, *, source: str) -> str:
    ...
    if len(key) < _MIN_KEY_LENGTH:
        raise RuntimeError(f"Samsung login-broker key fra {source} er for kort")
    return key
...
def broker_key() -> str:
    ...
    Existing deployments can continue to provide SAMSUNG_LOGIN_BROKER_KEY.
    Otherwise both the mobile API and isolated login broker share the same
    random key through their existing persistent /data volume. O_EXCL makes
    first-start generation safe even when both containers start concurrently.
    """
    configured = os.getenv("SAMSUNG_LOGIN_BROKER_KEY", "").strip()
    if configured:
        return _validated(configured, source="SAMSUNG_LOGIN_BROKER_KEY")

    path = broker_key_path()
    try:
        return _validated(path.read_text("utf-8"), source=str(path))
